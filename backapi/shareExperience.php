<?php 
	...
	if(mysqli_connect_errno($connection)){
		...
		$sql = "INSERT INTO `experience` (`username`, `title`, `content`, `date`) VALUES('$username', '$title','$content', '$date')";
		$result = mysqli_query($connection,$sql);
		if($result){
			$data = array('code'=>1,'mesg'=>"分享成功", 'date'=>$date);
		}else{
			$data = array('code'=>0,'mesg'=>"分享失败：".mysqli_error($connection), 'date'=>$date);
		}
	}
	// $jsonp="userData";
	// 转换格式
